...
class ClockTimeControl extends FlareControls {
  ...
  bool advance(FlutterActorArtboard artboard, double elapsed) {
    ...
    hourAnimation?.apply(
        (hourAnimation.duration * (dateTime.hour % 12) / 12).truncateToDouble(),
        artboard,
        1.0);
    minuteAnimation?.apply(
        (minuteAnimation.duration * dateTime.minute / 60).truncateToDouble(),
        artboard,
        1.0);
    return true;
  }
...
class ClockHandControl extends FlareControls {
  ...
  bool advance(FlutterActorArtboard artboard, double elapsed) {
    ...
    minuteAnimation?.apply(
        minuteAnimation.duration * minute / 60, artboard, 1.0);
    secondAnimation?.apply(
        secondAnimation.duration * second / 60, artboard, 1.0);
    return true;
  }
